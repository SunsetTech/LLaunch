		...
		Name, nil, {
					...
					for _, Collection in pairs(Config.Collections) do
						if Collection.Label == Details[1] then
							for _, LauncherName in pairs(Collection.Launchers) do
								LauncherSelect:Add(LauncherName,UITree.Output.Text("Name",nil,LauncherName),LauncherName)
							end
			...
			UITree.Input.Action(
				...
				"Add Games", function()
					...
					for _, Collection in pairs(Config.Collections) do
						CollectionSelection:Add(Collection.Label, UITree.Output.Text(Collection.Label, nil, Collection.Label), Collection.Label) --TODO this sucks
					end
					return CollectionSelection, function()
						...
						for _, GameOption in pairs(CollectionCard:Find"Game".Children) do
							GameSet:Add(UITree.Input.Boolean(GameOption.Name, nil, false))
						end
						return GameSet, function()
							...
							for _, GameEntry in pairs(GameSet.Children) do
								if GameEntry:GetValue() then
									...
									AppSelection:Add(
										GameEntry.Name,
										UITree.Output.Text("Name", nil, GameEntry.Name),
...
return function(Config, Oops)
	...
	for Row in Oops.Database:nrows("SELECT DISTINCT List FROM ListEntries") do
		ListSelection:Add(Row.List, UITree.Output.Text("Name", nil, Row.List), Row.List)
		...
		for Row in SelectStatement:nrows() do
			Card:Find"Game/App":Add(
				Row.Identifier,
				UITree.Output.Text("Name", nil, Row.Name),
